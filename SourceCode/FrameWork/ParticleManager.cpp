...
void ParticleManager::ParticleManagerCommon(ID3D12Device* dev, ID3D12GraphicsCommandList* cmdList,Camera* camera) {
	//nullptrチェック
	assert(dev);
	assert(cmdList);
	assert(camera);
...
std::unique_ptr<ParticleManager> ParticleManager::Create(std::wstring fName) {
	// nullptrチェック
	assert(device);
	assert(camera);
	//インスタンスを生成
	ParticleManager* partMan = new ParticleManager();
	if (partMan == nullptr) {
		return nullptr;
	}
...
void ParticleManager::CreateAlphaBlendPipeline() {
	HRESULT result = S_FALSE;
...
void ParticleManager::CreateAddBlendPipeline() {
	...
	result = D3DX12SerializeVersionedRootSignature(&rootSignatureDesc, D3D_ROOT_SIGNATURE_VERSION_1_0, &rootSigBlob, &errorBlob);
	// ルートシグネチャの生成
	result = device->CreateRootSignature(0, rootSigBlob->GetBufferPointer(), rootSigBlob->GetBufferSize(), IID_PPV_ARGS(&addBlendPipelineSet.rootsignature));
	if (FAILED(result)) {
		assert(0);
	}
...
void ParticleManager::DrawPrevAlphaBlend() {
...
void ParticleManager::LoadTexture(std::wstring fName) {
	...
	std::wstring name = L"Resources/2d/Effect/" + fName + L".png";
