...
void MThirdStage::Initialize(DirectXCommon* dxCommon) {
	InitCommon(dxCommon);
	BattleInit();
...
//開放処理
void MThirdStage::Finalize() {
	//３ｄのモデルのデリート
	ActorManager::GetInstance()->Finalize();
}
//更新
void MThirdStage::Update(DirectXCommon* dxCommon) {
	//導入部分の処理
...
//描画
void MThirdStage::Draw(DirectXCommon* dxCommon) {
	dxCommon->PreDraw();
